좋습니다. 지금 로그가 **anon-rss ≈ 60.0GB(62,959,908kB)** 에서 죽었죠. 64GB 머신에서 OOM-killer가 뜨는 건 “사용 가능한 RAM”이 커널/캐시/다른 프로세스 때문에 **대략 55~61GB 근처로 줄기 때문**이고요.

즉 **지금 상태에서 “추가로 6~10GB 정도”만 피크를 깎으면 64GB에서도 돌아갈 가능성이 매우 높습니다.**
가장 큰 피크는 보통:

1. `model.safetensors` → (당신은 여기서 이미 한 번 죽었음)
2. **Sampling 0% 직전**: 첫 forward 직전/중에 **CPU에 남아있는 로드 잔여 텐서 + T5(텍스트 인코더) + 기타 버퍼**가 겹쳐서 순간 피크가 튐

그래서 아래처럼 고치겠습니다.

## 핵심 변경 3가지 (64GB용)

1. **safetensors를 CPU가 아니라 “GPU로 직접 로드”** (`safe_open(device="cuda:0")`)
   → CPU에 “키별 텐서”가 안 올라오니 RAM 피크가 확 줄어듭니다.
2. **T5는 인코딩 끝나면 바로 unload(메모리 해제)**
   → 몇 GB 단위로 내려가는 경우 많습니다.
3. 모델 로딩/레이어 주입 루프에서 **강제 GC + 텐서 참조 끊기** (피크 완화)

아래는 **바로 복붙 가능한 전체 코드 2개 파일**입니다.
...
# 1) `load_prequant.py` (전체 코드로 덮어쓰기)

> ✅ 변경점: `safe_open(... device=target_device)`로 **GPU 직접 로드** (CPU RAM 피크 급감)
> ✅ 레이어마다 `del` + `gc.collect()`로 잔여 참조 즉시 정리
...
def reconstruct_params4bit_from_components(
    ...
    """
    NOTE: weight_components tensors are already on target device if safetensors safe_open(device=target).
    """
    qs_dict = {
    ...
    quantized_weight = weight_components["weight"]  # already on device
...
def _stream_load_safetensors_direct_to_device(
    model: nn.Module,
    weights_path: str,
    layer_shapes: Dict[str, Tuple[int, int]],
    device: str,
    ...
    """
    ✅ 64GB RAM 핵심 최적화:
    - safe_open을 target device(cuda:0)로 열어서, 텐서가 CPU RAM에 안 올라오게 함
    - 레이어별로 바로 Params4bit로 만들어 module.weight에 꽂고, 즉시 GC
    """
    ...
    # ✅ 여기서 device를 cuda로 주면 get_tensor가 GPU 텐서로 나옵니다(= CPU RAM 피크 감소)
    safetensors_device = device if device.startswith("cuda") else "cpu"

    loaded_count = 0
    with safe_open(weights_path, framework="pt", device=safetensors_device) as f:
        keys = set(f.keys())
        ...
        for name, module in model.named_modules():
            ...
            if k_bias in keys and module.bias is not None:
                module.bias.data.copy_(f.get_tensor(k_bias))

            # ✅ 강제 해제(피크 완화)
            del comps, param
            if loaded_count % 64 == 0:
                gc.collect()

        print(f" Loaded {loaded_count} quantized linear layers (direct-to-{device})")
        ...
        for k in keys:
            ...
            if k in param_map:
                ...
                dst.data.copy_(t.to(dst.device) if t.device != dst.device else t)
...
def load_quantized_state(
    ...
    if weights_path.endswith(".safetensors"):
        return _stream_load_safetensors_direct_to_device(model, weights_path, layer_shapes, device=device)
        ...
        if isinstance(module, bnb.nn.Linear4bit):
            if name in weight_components and name in layer_shapes:
                ...
                if "weight" in components:
                    # NOTE: pt fallback는 CPU->GPU copy unavoidable
                    for ck, cv in components.items():
                        if torch.is_tensor(cv):
                            components[ck] = cv.to(device)
...
def load_quantized_model(model_dir: str, device: str = "cpu") -> WanModel:
    ...
    # ✅ 중요: weights를 target device로 직접 스트리밍 로드
    model = model.to(device if not device.startswith("cuda") else "cpu")  # 구조만 먼저 잡기(안전)
...
# 2) `generate_prequant_64gb.py` (전체 코드로 저장)

> ✅ 변경점:
>
> * diffusion 모델은 **1개만 lazy-load** 유지
> * **T5 인코딩 끝나면 즉시 unload** (`del self.text_encoder.model` + gc)
> * 나머지는 이전 VRAM 피크 감소(in-place CFG, fp32 큰 텐서 제거) 유지
...
class WanI2V_PreQuant:
    ...
    def _load_model(self, which: str):
        ...
        # ✅ weights는 load_prequant에서 cuda로 직접 스트리밍 로드 (CPU RAM 피크 줄이기)
        logger.info(f"Loading {which} model to GPU (direct streaming): {model_dir}")
    ...
    def _encode_text_and_free_t5(self, input_prompt: str, n_prompt: str):
        """
        ✅ 64GB 핵심: 프롬프트 임베딩 생성 후 T5를 메모리에서 내려 RAM 확보
        """
        if not self.t5_cpu:
    ...
    def generate(
        ...
        # ✅ 텍스트 인코딩 후 T5 언로드
        context, context_null = self._encode_text_and_free_t5(input_prompt, n_prompt)
        ...
        # ✅ VAE encode 준비 (CPU 왕복 금지 + zeros fp16/bf16 GPU)
...
```bash
python generate_prequant_64gb.py \
  --ckpt_dir . \
...
* **CPU RAM 피크에서 가장 큰 덩어리(가중치 텐서들)를 GPU로 “직접” 읽게 해서**
  기존 60GB 피크가 **몇 GB~수십 GB까지도 내려가는 케이스가 있습니다.**
* **T5 언로드**는 환경에 따라 **2~6GB 정도** 내려가는 경우가 흔합니다.

---

## 만약 그래도 죽으면(다음 “확정” 단계)

바로 이 두 줄만 보면 어디가 병목인지 확정됩니다.
...
특히 `anon-rss`가 **여전히 60GB 근처**면 “남은 CPU 피크 원인”이 따로 있고(예: 다른 프로세스/캐시/overcommit), 그때는 **커널 설정(Overcommit)로 OOM-killer 회피**가 더 확실한 해결책입니다.
